...
class JobService:
    ...
    def _run_job(self, job: OCRJobRequest) -> None:
        try:
            ...
            pipeline_output = self.document_pipeline.execute(
                {
                    "document_id": job.document_id,
                    "document_type": job.document_type,
                    "pdf_path": downloaded_path,
                    "output_root": self.storage_service.output_root / job.document_id,
                }
            )
            logger.info("Merged OCR completed document_id=%s", job.document_id)

            callback_payload = self._build_callback_payload(job, pipeline_output)
